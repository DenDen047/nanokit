...
ユーザーの好み・背景・作業スタイルを**能動的に収集し、全プロジェクトの回答に反映する**。

**保存はその都度・即時に（セッション終了を待たない）。** ユーザーが恒久的な好み・個人的事実・作業スタイル・行動への訂正を示したら、**その場で**保存する。これが本命の仕組みで、Stop フックは取りこぼし防止の保険にすぎない。ルーティング:

- **横断的に効く個人の事実/好み** → **グローバル個人メモリ** `~/.claude/memory/personal/` に 1 ファイル 1 事実（kebab-case slug ＋ frontmatter `name`/`description`/`metadata: type`）で追記し、`MEMORY.md` にポインタを 1 行足す。下の `@import` で全プロジェクトに読み込まれる。
- **そのプロジェクト固有の事実** → そのプロジェクトの `memory/`（ネイティブ auto-memory）。
- **確定した好みの CLAUDE.md への昇格**は人手で（スクリプトは自動編集しない）。

「個人 vs プロジェクト」で迷ったら、**他プロジェクトでも役立つならグローバル個人メモリ**へ。既に記録済みのものは再保存しない。

セッション終了時には `Stop` hook (`memory-extract-reminder.sh`) が取りこぼし防止のナッジを出す（**session_id 単位**で約15分ごとに最大1回・非ブロッキング）。
