...
- **散文は 1 段落 = 1 行に畳む**。ターミナル表示の幅を抑えるためだけに段落の途中へ入った改行は、コピー時に取り除いて 1 行にする (改行を消した箇所は日本語なら詰め、英語なら半角スペース 1 つで繋ぐ)。貼り付け先が勝手に折り返すので、見た目の改行は不要どころか邪魔になる。
- ただし **構造上の意味がある改行は必ず残す**。空行 (段落の区切り)、箇条書き・番号付きリストの各項目、見出し、表の各行、コードブロックやコマンド、YAML / JSON などの中身は一切触らない。
